...
describe('UI tests for sign in page', () => {
    ...
    it("should show 'Real World App logo'", () => {
        cy.get(selectors.logoImage).should('be.visible').and('have.attr', 'xmlns', 'http://www.w3.org/2000/svg')
    })

    it("should show 'Sign in' title", () => {
        cy.get(selectors.titleText).should('be.visible').and('have.text', 'Sign in')
    })
    ...
    it("check 'Remember me' checkbox", () => {
        cy.get(selectors.clickCheckboxOn).check().should('be.checked')
        cy.get(selectors.clickCheckboxOff).uncheck().should('not.to.be.checked')
    })

    it("should show enabled by default 'sign in button'", () => {
